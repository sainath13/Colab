...
class SearchPage extends Component {
  ...
  componentDidMount() {
    ...
    this
      .props
      .fetchTrending(this.props.loginInfo.id)
      .then((res) => {
        this
          .props
  ...
  searchNichePressed(nichename) {
    this
      .refs
      .textInput1
      .setNativeProps({text: nichename})
    this.setState({searching: true, showingSearchResults: true, searchInput: nichename});
    this
      .props
      .fetchSearch(this.props.loginInfo.id, nichename, this.state.isNameSelected)
      .then((res) => {
  ...
  searchPressed() {
    ...
    this
      .props
      .fetchSearch(this.props.loginInfo.id, this.state.searchInput, this.state.isNameSelected)
      .then((res) => {
        this.setState({searching: false});
      });
  }
  fetchTrendingItems() {
    return Object
      .keys(this.props.trendingData)
      .map(key => this.props.trendingData[key]);
  }
  fetchNicheItems() {
    return Object
  ...
  fetchSearch() {
    return Object
      .keys(this.props.searchData)
      .map(key => this.props.searchData[key]);
  }

  selectName() {
    this.setState({isNameSelected: true, isNicheSelected: false})
  }
  selectNiche() {
    this.setState({isNameSelected: false, isNicheSelected: true})
  }
              ...
              <View style={{
                ...
                <View style={styles.contentPic}>
                  <View
                    ...
                    <TouchableHighlight
                      style={[
                      styles.nicheContainer, this.state.isNicheSelected && styles.nicheContainerHighlighted
                    ]}
                      onPress={() => this.selectNiche()}>
                      ...
                      <View style={styles.searchBarOutside}>
                        <View style={styles.searchBarInside}>
                          <TextInput
                            ...
                            onChangeText={(searchInput) => this.setState({searchInput})}></TextInput>
                        </View>
                        <View style={styles.searchIcon}>
                          <TouchableHighlight onPress={() => this.searchPressed()}>
                            <Icon name="search" size={30} color='#58568f'></Icon>
                          </TouchableHighlight>
                        </View>
                      </View>
                      <TouchableHighlight
                        ...
                        <View
                          ...
                          <Icon name="times-circle-o" size={30} color='#58568f'></Icon>
                        </View>
                      </TouchableHighlight>
                    </View>
                  </View>
                ...
                {this.state.searching || this.state.loading || this.state.loadingNiche
                  ? <View
                      ...
                      <Spinner
                        ...
                        isVisible={this.state.searching || (this.state.loading && this.state.loadingNiche)}
                        size={50}
                        type={'ThreeBounce'}
                        color={'#65634A'}/>
                    </View>
                  ...
                  <View style={{
                    ...
                    {!this.state.showingSearchResults && this.state.isNicheSelected && !this.state.loadingNiche
                      ? <View
                          ...
                          {!this.state.showingSearchResults && this.state.isNicheSelected && !this.state.loadingNiche && this
                            .fetchNicheItems()
                            .map((nicheItem) => {
                              return (
                                <TouchableHighlight
                                  ...
                                  this.searchNichePressed(nicheItem.name)
                                }}
                                  style={{
                                  marginTop: 5,
                                  marginBottom: 5,
                      ...
                      : null}
                    {!this.state.showingSearchResults && this.state.isNameSelected && !this.state.loading && this
                      .fetchTrendingItems()
                      .map((searchItem) => {
                        return (
                          <TouchableHighlight
                    ...
                    {!this.state.searching && this.state.showingSearchResults && this
                      .fetchSearch()
                      .map((searchItem) => {
                        return (
                          <TouchableHighlight
                            key={searchItem.id}
                            onPress={() => {
                            Actions.VisitProfilePage({clickedUserId: searchItem.id, isBusiness: false})
                          }}>
                            <View
                              style={{
                              flex: 1,
...
var styles = StyleSheet.create({
  ...
  contentPic: {
    flex: 2,
    backgroundColor: '#6563A4',
    // flexDirection : 'row', borderColor : 'red', alignItems: 'space-around',
  ...
  nicheContainer: {
    flex: 1,
    // backgroundColor : '#f3f3f1', backgroundColor : 'red',
    margin: 5,
    marginLeft: 20,
    marginRight: 20,
    borderRadius: 5,
